(ns rd-management.actor
  "RDManagementActor — the ISCO-08 1223 independent research-and-
  development-management actor as a `langgraph.graph/state-graph` (per
  ADR-2607011000 / CLAUDE.md Actors section). One graph run = one R&D
  ...
  The unconditional invariant: the RDAdvisor can never directly commit a
  record or dispatch a robot action the RDManagementGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [rd-management.advisor :as advisor]
            [rd-management.governor :as governor]
            [rd-management.store :as store]))

(defn build-graph
  "Build a compiled RDManagementActor graph. `store` implements
  `rd-management.store/Store`. `advisor` implements
  `rd-management.advisor/Advisor` (defaults to `mock-advisor`).
